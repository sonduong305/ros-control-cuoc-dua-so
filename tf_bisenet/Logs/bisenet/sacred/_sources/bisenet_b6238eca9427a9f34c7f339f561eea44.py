...
class BiseNet(object):
    ...
    def build_loss(self):
        ...
        loss1 = tf.losses.mean_squared_error(self.fc, self.regression_label)
        ...
        tf.summary.image('GT', tf.reshape(
            tf.matmul(tf.reshape(self.labels, [-1, 7]), colors), [-1, shape[1], shape[2], 3]),
