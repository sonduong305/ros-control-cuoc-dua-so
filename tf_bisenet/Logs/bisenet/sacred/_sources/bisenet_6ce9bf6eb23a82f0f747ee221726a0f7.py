...
class BiseNet(object):
    ...
    def build_loss(self):
        ...
        tf.summary.image('GT', tf.reshape(
            tf.matmul(tf.reshape(self.labels, [-1, 14]), colors), [-1, shape[1], shape[2], 3]),
                         family=self.mode, max_outputs=1)
        tf.summary.image('response', tf.reshape(tf.matmul(
            tf.reshape(tf.one_hot(tf.argmax(self.net, -1), self.num_classes), [-1, 14]), colors),
